---
title: Terms of Service — SpeedSync
permalink: /TERMS/
---

# Terms of Service

**SpeedSync (Android Application)**
**Effective Date:** 2026-04-26
**Contact:** support@speedsync.dev

---

## 1. Acceptance of Terms

By using SpeedSync, you agree to these Terms.

If you do not agree, do not use the application.

---

## 2. Description of Service

SpeedSync is a utility that enables users to transfer their own workout
data from Speediance to Garmin Connect.

The application operates entirely on the user's device and does not
provide cloud services.

---

## 3. Independent Service

SpeedSync is **not** affiliated with, endorsed by, or sponsored by:

- Garmin
- Speediance

All trademarks belong to their respective owners.

---

## 4. User Responsibility

You agree to:

- use the application only for your own accounts
- maintain the security of your device
- comply with applicable laws and third-party terms

---

## 5. Third-Party Services

SpeedSync interacts with third-party services at your request.

We are not responsible for:

- availability of these services
- changes to their APIs or policies
- interruptions or failures in their systems

---

## 6. Service Dependency

SpeedSync depends on third-party services.

If those services change or become unavailable:

- some or all features may stop working
- restoration is not guaranteed

---

## 7. Purchases

SpeedSync is a free application. The first three syncs are included at
no cost. Unlimited syncs are available via an optional one-time in-app
purchase ("Pro unlock") through Google Play. There is no subscription
and no recurring charge.

Payments are processed by Google Play. We do not handle payment
information.

Purchases are subject to Google Play's refund policies.

---

## 8. No Warranty

The application is provided "as is" without warranties of any kind.

We do not guarantee:

- uninterrupted service
- compatibility with third-party services
- error-free operation

---

## 9. Limitation of Liability

To the maximum extent permitted by law:

- we are not liable for indirect or consequential damages
- we are not responsible for data loss caused by third-party services
- total liability is limited to the amount paid for the app

---

## 10. Termination

You may stop using the app at any time by uninstalling it.

We may discontinue the service at any time.

---

## 11. Governing Law

These Terms are governed by the laws of the Province of Quebec, Canada,
unless otherwise required by mandatory consumer protection laws in your
jurisdiction.

---

## 12. Changes

We may update these Terms. Continued use of the app constitutes
acceptance of changes.

---

## 13. Contact

support@speedsync.dev
